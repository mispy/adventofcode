...
/** @param {string[]} lines */
function main(lines) {
    const valid = lines.map(aoc.words).filter(words => _.uniq(words).length === words.length)
    log(valid.length)

    const valid2 = lines.map(aoc.words).filter(words => _.uniq(words.map(w => _.sortBy(w).join(""))).length === words.length)
    log(valid2.length)
}
