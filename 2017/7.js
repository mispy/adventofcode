...
function main(lines) {
    ...
    for (let line of lines) {
        let [p, s] = line.split(/ -> /)
        let name = p.split(" ")[0]
        let weight = aoc.ints(p)[0]
        let deps = s ? s.split(", ") : []
        pros[name] = { name, weight, deps }
    }

    for (let pro of _.values(pros)) {
        for (let dep of pro.deps) {
            pros[dep].parent = pro
        }
    }

    function sumWeight(node) {
        let childNodes = node.deps.map(d => pros[d])
        let sums = childNodes.map(n => sumWeight(n))
        return node.weight + _.sum(sums)
    }
    ...
    function findUnbalanced(node) {
        ...
        if (_.uniq(sums).length !== 1) {
            // One of them is unbalanced
            childNodes.map(findUnbalanced)
        } else {
            // It might be this node!
            const siblings = node.parent.deps.map(d => pros[d])
            let sibsums = siblings.map(s => sumWeight(s))
            if (_.uniq(sibsums).length !== 1) {
                log(sibsums, siblings.indexOf(node), node.weight)
            }
        }
/*        if (sums.length > 0 && _.uniq(sums).length !== 1) {
            childNodes.map(c => console.log(c.weight, sum(c)))
        }
        return node.weight + _.sum(sums)*/
    }
    

    let root = _.filter(pros, pro => !pro.parent)[0]
    log(root)
    log(findUnbalanced(root))
}
