...
function partOne(lines: string[]) {
    let d = lines.map(l => {
        let spl = l.split(" ")
        let date = moment((spl[0] + " " + spl[1]).slice(1, -1))
        return { date: date, line: l }
    })

    d = _.sortBy(d, l => l.date) as { date: any, line: string }[]

    let guards = {}
    let guard = null
    let sleepStart: moment.Moment|null = null
    for (const l of d) {
        let m = l.line.match(/Guard #(\d+)/)
        if (m) {
            sleepStart = null
            guard = m[1]
            if (!guards[guard])
                guards[guard] = { id: guard, total: 0, minutes: [] }
        } else if (l.line.match(/falls asleep/)) {
            sleepStart = l.date
        } else if (sleepStart !== null && l.line.match(/wakes up/)) {
            const minutes = moment.duration(l.date.diff(sleepStart)).asMinutes()
            guards[guard].total += minutes
            for (let i = 0; i < minutes; i++) {
                const minute = sleepStart.clone().add(i, 'minutes').get("minutes")
                guards[guard].minutes.push(minute)
            }
        }
    }

    let sleepy = _.sortBy(_.values(guards), g => -g.total)[0]

/*    var result = _.head(_(sleepy.minutes)
    .countBy()
    .entries()
    .maxBy('[1]'));*/
    let times = _.sortBy(_.entries(_.countBy(sleepy.minutes)), a => -a[1])
    log(times[0], times[1])
    log(parseInt(times[0] as any) * sleepy.id)
    log(sleepy.id)
}
...
function partTwo(lines: string[]) {
    ...
    for (let guard of _.values(guards)) {
        guard.times = _.sortBy(_.entries(_.countBy(guard.minutes)), a => -a[1])
        if (guard.times.length) {
            guard.maxMinute = parseInt(guard.times[0][0])
            guard.maxSleep = guard.times[0][1]
    
        }
    }

    let g = _.sortBy(_.values(guards), g => -g.maxSleep)[0]
    console.log(g.id, g.maxMinute, g.maxSleep, g.id*g.maxMinute)
