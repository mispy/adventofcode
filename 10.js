...
function getBounds(points) {
    let minX = _.min(points.map(c =>c.pos.x))
    let maxX = _.max(points.map(c =>c.pos.x))
    let minY = _.min(points.map(c => c.pos.y))
    let maxY = _.max(points.map(c => c.pos.y))
...
function renderPoints(points) {
    ...
    const grid = [...Array(5+maxY-minY)].map(x=>Array(5+maxX-minX).fill("."))

    for (const point of points) {
        let y = point.pos.y-minY
        let x = point.pos.x-minX
        grid[point.pos.y-minY][point.pos.x-minX] = "#"
    }
...
function main(lines) {
    const points = lines.map(l => {
        ...
        return {
            pos: new Vector2(ints[0], ints[1]),
            vel: new Vector2(ints[2], ints[3])
        }
    })
