...
function main(input) {    
    ...
    function parseGroup(chars) {
        ...
        _.times(chars.length, i => {
            ...
            if (ch === ")" && openGroup === 1) {
                path.push(parseGroup(subgroup))
                subgroup = null
            } else if (openGroup > 0) {
                subgroup.push(ch)
            } else if (ch === "(" && openGroup === 0) {
                subgroup = []
            } else {
                if (ch === "|") {
                    group.options.push(path)
                    path = []
                } else {
                    path.push(ch)
                }
    ...
    const path = parseGroup(Array.from(input).slice(1, -1)).options[0]

    function longestPath(path) {
        let result = []
        for (const child of path) {
            if (_.isString(child))
                result.push(child)
            else {
                if (_.some(child.options, opt => opt.length === 0)) {

                } else {
                    const dirs = child.options.map(path => longestPath(path))
                    result.push(..._.sortBy(dirs, p => -p.length)[0])    
                }
            }
        }
        return result
    ...
    function pathfind(path, x, y, len) {
        for (const child of path) {
            if (_.isString(child)) {
                const ch = child
                if (ch === 'N')
                ...
                let key = `${x},${y}`
                let current = shortestPaths[key]
                if (!current || len < current)
                    shortestPaths[key] = len
            } else {
                child.options.map(opt => pathfind(opt, x, y, len))
            }
        }
    }
